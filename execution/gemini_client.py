...
def _try_model(model: str, payload: bytes, api_key: str, timeout: int,
               retries: int, backoffs: list[int]) -> str | None:
...
def call_gemini(prompt: str, api_key: str, temperature: float = 0.5,
    ...
    result = _try_model(
        FLASH_MODEL, payload, api_key, timeout,
    ...
    result = _try_model(
        PRO_MODEL, payload, api_key, timeout,
