...
**Latest checkpoint:** `.tmp/checkpoint_2026-04-15_session52.md` — start here. (Predecessors: s51, s50, s49, s48, s47, s46, s45, s44, s43, s42, s41, s40, s39, s38, s37, s36, s35, s34, s33, s32, s31.)

- **Session 52 — Error-surfacing MVP (DEPLOYED, commit `dd5f311`).** One deploy bundled s51 JSON-retry + IMAGEN_FAILED recovery + empty-scene guard + `PYTHONUNBUFFERED=1` Fly secret. Key additions: `TransientUpstreamError` in `http_retry.py` makes Imagen silent-safety-filter drops (HTTP 200 + empty `predictions`) retryable; `friendlyErrorMessage()` in studio template + `ERROR_CODE_COPY` in `/runs/[id]` map all 5 pipeline error codes to plain English; Try Again + Start Over redirect to `IGLOO_APP_URL` (both previously re-acquired failed run_id); `_first_empty_narration()` + `validate_scenes` tightening catch Gemini narration drift before it reaches the browser, with one-shot Pro-forced retry on `/api/assemble-script` + `/api/edit-script`. **Shipped without end-to-end verification** — worth smoke-testing on prod. **Deferred:** "pick different presenter" UX (re-pick from other 2 candidates instead of new topic); Supabase status-sync inconsistency (some Fly failures sync `rejection_reason`, others don't).
- **Session 51 — Rolling JSON-retry hardening (shipped with s52 batch).** `call_gemini_json()` + `GeminiJSONError` in `gemini_client.py` (3 attempts, last escalates to Pro) wrapping `/api/edit-narration`, `/api/generate-characters`, `/api/custom-character` — friendly 502 on JSON-parse failure instead of leaking Python tracebacks.
