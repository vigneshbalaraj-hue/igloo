...
**Latest checkpoint:** `.tmp/checkpoint_2026-04-15_session51.md` — start here. (Predecessors: s50, s49, s48, s47, s46, s45, s44, s43, s42, s41, s40, s39, s38, s37, s36, s35, s34, s33, s32, s31.)

- **Session 52 — Error-surfacing MVP (DEPLOYED with s51 batch).** Three bundles in one deploy: (1) s51 JSON-retry (`call_gemini_json` + friendly 502 on 3 light endpoints). (2) IMAGEN_FAILED MVP on cold-shower reels — Imagen silent-safety-filter drop (HTTP 200 + `predictions: []`) on anchor portraits now retries 3× via new `TransientUpstreamError` in `http_retry.py`; studio + `/runs/[id]` map `IMAGEN_FAILED`/`TTS_FAILED`/`KLING_FAILED`/`MUSIC_FAILED`/`FFMPEG_TIMEOUT` → plain-English; Try Again + Start Over both redirect to `IGLOO_APP_URL` (both were broken — Flask session still held failed run_id, re-acquire refused). (3) Empty-scene bug fix — Gemini drift sometimes returned a scene with blank `narration_text` rendering as empty quotes. Tightened `validate_scenes` to flag empty strings; added `_first_empty_narration()` guard in `web_app.py` with one-shot retry on `/api/edit-narration` (via `call_gemini_json` auto-escalation) and `/api/assemble-script` + `/api/edit-script` (forced to Pro via `force_model=PRO_MODEL`). Also shipped `PYTHONUNBUFFERED=1` Fly secret (s48 deferred). **Not locally verified** — shipped unverified after weighing risk (fallthrough paths cover the main endpoints; studio template JS is the only real blast-radius risk). **Deferred:** "pick different presenter" UX on anchor-only Imagen failure (parked). Also flagged: some Fly pipeline failures DO sync `runs.status`/`rejection_reason` (cold-shower), others don't (F1) — intermittent, needs investigation.
- **Session 51 — Rolling JSON-retry hardening.** Kaushik's F1 pit-stop topic hit two errors. (1) `IMAGEN_FAILED` in studio overlay — diagnosed only at the time. (2) `JSONDecodeError` raw Python error leaked to user on "Apply Edit" — added `call_gemini_json()` + `GeminiJSONError` in `execution/gemini_client.py` (retry ×3, last attempt escalates to Pro). Three light endpoints use it and return friendly 502. Shipped with s52 batch.

- **Session 50 — Supabase cap + dabbawala salvage.** Dabbawala run (`76d7b22a...`) failed upload with 413 — `reels` bucket capped at 50MB, MP4 was 51.7MB. User raised both global + bucket-level limits to 200MB in Supabase dashboard. Salvaged MP4 from Fly ephemeral disk via `flyctl ssh console` + manual `upload_final_to_supabase()` call; run flipped to `awaiting_review`. Underlying cause: `assemble_video.py` encodes at `-crf 18 -preset fast` at 5 sites — archival-grade, 2× too big for phone distribution. **Compression pass (CRF 23 preset medium, single shared constant) is top follow-up**. Also flagged: `upload_final_to_supabase` doesn't check run_id exists before uploading — silent orphan if run_id wrong. No code changes committed this session.
