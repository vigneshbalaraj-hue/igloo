...
**Latest checkpoint:** `.tmp/checkpoint_2026-04-14_session47.md` — start here. (Predecessors: s46, s45, s44, s43, s42, s41, s40, s39, s38, s37, s36, s35, s34, s33, s32, s31.)
...
- **Deferred:** Draft credit leak (s46 — section E: `redeem_credit()` fires before run enters pipeline, orphaned drafts keep the credit; needs move to `draft → queued/running` transition + nightly cron for >24h old drafts), silent-failure plan (s45 — approved, not executed), CDN for landing assets (~10MB after s44 compression), Supabase Storage cleanup cron (30-day expiry), email on deliver, Razorpay refund API, `run_pipeline.py:251` CLI `--speed` fix, Razorpay reconciliation cron.
